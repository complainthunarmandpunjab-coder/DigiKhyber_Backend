...
exports.login = async (req, res) => {
  try {
    ...
    const html = getUserLoginEmailHtml({
      ...
      dashboardUrl: "https://digikhyber.org.pk/login",
    });

    // Remove login email - as per requirements, don't send email on login
    // const emailResult = await sendEmail({
    //   email: user.email,
    //   subject: "Welcome back to Digikhyber!",
    //   html: html,
    //   emailType: 'contact',
    // });
    ...
    res.status(200).json({
      ...
      token,
      user: user, // Send full user object for dashboard
      challan: challanData,
    });
  } catch (error) {
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const resetUrl = `https://digikhyber.org.pk/reset-password/${resetToken}`;
    ...
    const emailResult = await sendEmail({
      email: user.email,
      subject: "Password Reset Request - Digikhyber",
      html: html,
      emailType: "contact",
    });
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const emailResult = await sendEmail({
      email: user.email,
      subject: "Password Changed Successfully - Digikhyber",
      html: html,
      emailType: "contact",
    });
